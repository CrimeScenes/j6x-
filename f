...
local function calculatePrediction(targetPlayer)
    ...
    -- Airborne targets: Predict parabolic movement
    if math.abs(velocity.Y) > 2 then
        local gravity = workspace.Gravity
        local timeInAir = velocity.Y / gravity  -- Approximate time airborne
        local futurePosition = humanoidRootPart.Position + velocity * timeInAir
        return futurePosition
    end

    -- Ground targets: Predict linear movement
    local futurePosition = humanoidRootPart.Position + velocity * ping
    return futurePosition
end
