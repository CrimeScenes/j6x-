...
local function calculatePrediction(targetPlayer)
    ...
    local ping = game:GetService("Stats"):FindFirstChild("PerformanceStats").Ping:GetValue() / 1000
    local gravityCompensation = Vector3.new(0, -workspace.Gravity * ping / 2, 0)

    -- Predict future position
    local futurePosition = humanoidRootPart.Position + velocity * ping + gravityCompensation
    return futurePosition
end

local function isWithinCursor(targetPlayer)
    ...
    -- Check if mouse overlaps any visible hitbox parts
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isWithinCursor(TargetPlayer) and aimAtTarget(TargetPlayer) then
                    ...
                    if tool and tool:IsA("Tool") then
                        if not isIgnoringKnife() then
                            ...
                            if shootFunction and shootFunction:IsA("RemoteEvent") then
                                ...
                                print("Fired using remote!")
                            else
                                local mouseX, mouseY = getMousePosition()
                                Forlorn.mouse1click(mouseX, mouseY.X, mouseY.Y)
                                print("Clicked mouse at target.")
                            end
                        end
                    else
                ...
                print("Cursor not on target.")
            end
        else
            print("Invalid target or target is knocked.")
        end
    end
end

-- Toggle TriggerBot with Keybind
